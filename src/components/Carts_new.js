...
const Cards_new = () => {
    ...
    let totDiscount = 0;
        items.forEach((item) => {
            totDiscount = amount(item.price) - total(item.price,item.discountPercentage)
        })
        ...
        <div className="flex flex-row ]">
            ...
            {isEmpty ? (
                ...
                items.map((item) => (
                    ...
                    <div key={item.id} className= "flex flex-row mb-3 h-[250px]" style={{ border: '1px solid #007bff', borderRadius: '0.25rem' }}>
                        ...
                        <div className="ml-10 mt-5 leading-[40px]">
                            ...
                            <div className="text-xl font-super-font">  {item.title}  </div>
                            <div> <h5> {item.brand} </h5></div>

                            <div className="flex flex-row ">
                                <div className=" mr-6"> <b> ₹{total(item.price,item.discountPercentage)} </b> </div>
                                <div style={{ color: 'red' }}> <del> ₹{amount(item.price)} </del> </div>
                            </div>

                            <div style={{ color: 'green' }}> <b>{item.discountPercentage}%off</b> </div>
                            ...
                            <div className="flex flex-row mt-5">
                                ...
                                <div className="flex flex-row bg-purple-500 rounded-lg  h-8 items-center">
                                    <img src="https://cdn-icons-png.flaticon.com/128/8997/8997755.png" alt="thunde" className="h-[20px] w-[20px] ml-4 mr-2" />
                                    <button className="text-white mr-6" onClick={() => handleRemoveItem(item.id)}> Remove Item </button>
                                </div>
                            </div>
            ...
            <div className="flex flex-col bg-zinc-200 w-[300px] mr-9 h-[300px] ">
                    <div className="flex h-9 ">
                        <div className="ml-[100px] mt-1" style={{color : 'grey'}}> <b> Price Details </b></div>
                    </div>
                    ...
                    <div className="flex flex-row ml-4 leading-10 mt-5">
                        ...
                        <div className="flex flex-col">
                            ...
                            <div className="mt-2"><b>Total Amout</b></div>
                        </div >

                        <div className="flex flex-col ml-[40px]">
                            <div><b>₹{tot}</b></div>
                            <div><b style={{color : 'green'}}>-₹{dis}</b></div>
                            <div   style={{color : 'green'}}> <del> {delivery}</del> <b>free</b></div>
                            <div className="mt-2"> <b> ₹{(amt)}</b></div>
                        </div>
